...
if [ ! -d "/var/lib/mysql/mysql" ]; then
	...
	setpriv --reuid=mysql --regid=docker-machines --init-groups -- mariadbd --defaults-file=/etc/my.cnf --skip-networking &
	jid="$!"
	for i in {1..30}; do
...
exec setpriv --reuid=mysql --regid=docker-machines --init-groups -- mariadbd --defaults-file=/etc/my.cnf
