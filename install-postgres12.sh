...
case "${DISTRIB_LABEL}" in
    debian*|ubuntu*)
        ...
        DEBIAN_FRONTEND=noninteractive apt-get -y -q update
        DEBIAN_FRONTEND=noninteractive apt-get install -y -q postgresql-common postgresql-12 postgresql-contrib-12
    ...
    centos8|rocky8)
        ...
        yum install -q -y postgresql12 postgresql12-server postgresql12-contrib
        yum clean all
        rm -rf /var/cache/yum
    ;;
    *)
