...
cleanup-docker-images: FORCE
	...
	-$(call task,removing latest tags matching dev images)
	-$(call irun,docker images --format "{{.Repository}} {{.ID}} {{.Tag}}" --filter "reference=optionfactory/*" | awk '{ if ($$3 ~ /-dev$$/) dev[$$2]=1; rows[NR]=$$0 } END { for (i=1;i<=NR;i++) { split(rows[i],f," "); if (f[3]=="latest" && (f[2] in dev)) print f[1]":latest" } }' | xargs --no-run-if-empty docker rmi)
	-$(call task,removing dev tags)
	-$(call irun,docker images --format "{{.Repository}}:{{.Tag}}" --filter "reference=optionfactory/*" | grep -- '-dev$$' | xargs --no-run-if-empty docker rmi)
